---
slug: /135
title: "React Hebdo #135: ..."
authors: [slorber]
date: 03-21-2023
image: /img/TWIR_POST.png
unlisted: true
---

Salut à tous !

React Hebdo vient de souffler sa 3ème bougie! J'ai fait un [post sur LinkedIn](https://link.slo.im/B87Zl) pour expliquer ou j'en suis. TLDR: la newsletter est enfin viable, avec plus de **17500 abonnés**, une belle croissance, et 27k€ de CA généré en 2022. Merci à vous de m'avoir soutenu dans le temps!

Le [Call for Paper](https://conference-hall.io/public/event/5hkuk4Ueg7U9y0uwNRRK) de la conf [React-Native-Connection](https://reactnativeconnection.io/) (1er Juin à Paris) est ouvert. N'hésitez pas à vous lancer. Le voyage et 2 nuits d'hôtel sont pris en charge pour les speakers.

Nouveau: si jamais vous recrutez un dev React (en CDI ou freelance), **[contactez-moi](https://qq4ph6n1rk0.typeform.com/to/qjSZ38dQ#email={{subscriber.email_address}})**.

<!-- truncate -->

**💡 [Consulter cette newsletter sur Twitter](https://slo.im/thread) - format visuel 🎨**

---

import Support from "../_support.mdx";

<Support />

---

## 💸 Sponsor {#sponsor1}

[![refine](/emails/issues/135/refine.png)](https://github.com/refinedev/refine)

[**refine - framework React headless pour créer des apps CRUD, 100% open-source**](https://github.com/refinedev/refine)

Jettez un oeil à [refine](https://github.com/refinedev/refine) avant de commencer votre prochain panneau d'administration, tableau de bord ou outil interne. Vous serez étonné de voir à quel point vous pouvez le développez plus vite, et de manière flexible.

---

## ⚛️ React {#react}

[useSignal() is the Future of Web Frameworks](https://www.builder.io/blog/usesignal-is-the-future-of-web-frameworks)

https://twitter.com/mhevery/status/1626269387438305280

https://twitter.com/acdlite/status/1626590880126889984

https://twitter.com/jordwalke/status/1626718819443150849

https://twitter.com/jordwalke/status/1626718819443150849

- [Angular Reactivity with Signals](https://github.com/angular/angular/discussions/49090) + https://twitter.com/angular/status/1625939902046117890 + https://twitter.com/sarah_edo/status/1628065696247857152

---

[![sandpack 2](/emails/issues/135/sandpack2.jpg)](https://codesandbox.io/blog/announcing-sandpack-2)

**[Announcing Sandpack 2.0 and a Node.js runtime for any browser](https://codesandbox.io/blog/announcing-sandpack-2)**

CodeSandbox annonce la nouvelle version de Sandpack, un outil qui permet de créer des expériences interactives de live-coding dans le browser (éditeur de code + preview). Sandpack s'intègre bien avec React via un package dédié. La v1 était déjà utilisée sur le nouveau site React, par Josh Comeau et bien d'autres.

Cette v2 repose sur la nouvelle technologie NodeBox qui permet de faire tourner Node.js dans le navigateur. Il est possible de faire tourner un meta-framework full-stack comme Next.js. Cela rappelle les WebContainers de StackBlitz, qui viennent d'ailleurs d'[annoncer la WebContainers API](https://blog.stackblitz.com/posts/webcontainer-api-is-here/). L'article explique les différences entre ces 2 technologies innovantes.

---

[The Power of Keys in Framer Motion](https://www.nan.fyi/keys-in-framer-motion)

---

---

[Leveraging Server Context for Advanced Next.js Optimization Patterns](https://prismic.io/blog/advanced-nextjs-server-context)

---

- 🐦 [Andrew Clark - "I'm joining Vercel"](https://twitter.com/acdlite/status/1623353741750546439)
- 👀 [React PR - Enable passing Server References from Server to Client](https://github.com/facebook/react/pull/26124): une PR intéressante qui permet aux composants client d'appeler des callbacks serveur, dans la tendance RPC actuelle.
- 👀 [TanStack Start](https://twitter.com/sebastienlorber/status/1627740526551175190): nouveau meta-framework React en cours de dev?
- 👀 [Next.js Route Handlers](https://twitter.com/nextjs/status/1626341664402051072)
- 👀 [RFC: React Aria Components](https://github.com/adobe/react-spectrum/pull/4058)
- 🧵 ["JSX could have been 2x faster if it was designed more optimally for JS VMs!"](https://twitter.com/mhevery/status/1626422858862960648)
- 🧵 [Dan Abramov - "20’s: Composing server and client as one paradigm"](https://twitter.com/dan_abramov/status/1625225990023655425)
- 📖 [Migrating to Modern Redux](https://redux.js.org/usage/migrating-to-modern-redux): nouvelle doc
- 📜 [Effective Higher-Order Components](https://www.bbss.dev/posts/effective-hocs/)
- 📜 [OSS Feature Decision Tree](https://tkdodo.eu/blog/oss-feature-decision-tree): Dominik explique comment il décide si une feature sera intégrée à React-Query. Cela me fait penser au [Extensible Web Manifesto](https://twitter.com/sebastienlorber/status/1627637410438815744).
- 📜 [Detecting Nested Components in React with the Context API](https://www.aleksandrhovhannisyan.com/blog/react-context-nested-components/)
- 📜 [Maximising performance with React code splitting techniques](https://edvins.io/react-code-splitting-techniques)
- 📜 [How to handle errors in React](https://www.developerway.com/posts/how-to-handle-errors-in-react)
- 📜 [Modularizing React Applications with Established UI Patterns](https://martinfowler.com/articles/modularizing-react-apps.html)
- 📜 [A Historical Reference of React Criticism](https://www.zachleat.com/web/react-criticism/)
- 📜 [A minimal, multi-framework, responsive image component](https://dev.to/ascorbic/a-minimal-multi-framework-responsive-image-component-3iop)
- 📜 [The case for frameworks](https://seldo.com/posts/the_case_for_frameworks)
- 📜 [Using WebSockets in a Redux Application](https://www.taniarascia.com/websockets-in-redux/)
- 📜 [How to Integrate GreenSock with Remotion](https://enlear.academy/how-to-integrate-greensock-with-remotion-e4eee6f5a41f)
- 📜 [Improved type safety in Storybook 7](https://storybook.js.org/blog/improved-type-safety-in-storybook-7/)
- 📜 [Hydrogen v2 is Remixed](https://twitter.com/remix_run/status/1623744656701026306)
- 📜 [Behind the Scenes with React.js: the Documentary](https://newsletter.pragmaticengineer.com/p/react-documentary)
- 📜 [Adopting React in the Early Days](https://cult.honeypot.io/reads/adopting-react-in-the-early-days/)
- 📜 [What Netlify's Acquisition of Gatsby Really Means](https://remotesynthesis.com/blog/gatsby-plus-netlify/)
- 🎥 [React.js: The Documentary](https://www.youtube.com/watch?v=8pDqJVdNa44)
- 🎥 [Q&A with the cast of React.js: The Documentary](https://www.youtube.com/watch?v=WzRSysq7F4k)
- 🎥 [Fireship - 7 better ways to create a React app](https://www.youtube.com/watch?app=desktop&v=2OTq15A5s0Y)
- 🎥 [Killing ReactJS - A Guide For Hopeful Frameworks](https://www.youtube.com/watch?v=LMUKsAf2Rfs)
- 📦 [Remix 1.13](https://github.com/remix-run/remix/releases/tag/remix%401.13.0): support Tailwind
- 📦 [MDX 2.3](https://github.com/mdx-js/mdx/releases/tag/2.3.0): nouvelles options du compiler pour le support d'autres frameworks que React
- 📦 [Unovis - modular data visualization framework for React, Angular, Svelte...](https://unovis.dev/)
- 📦 [Redwood 4.1](https://github.com/redwoodjs/redwood/releases/tag/v4.1.0)

<SubscribeFormEmbed />

---

## 💸 Sponsor {#sponsor2}

[![surveyjs.jpg](/emails/issues/133/surveyjs.jpg)](https://surveyjs.io/)

**[Librairie JavaScript extensibles pour les sondages et les formulaires](https://surveyjs.io/)**

[SurveyJS](https://github.com/surveyjs) est un ensemble de librairies de formulaires JavaScript open-source que vous pouvez facilement intégrer à votre application Web. Cela vous permet de **créer et d'exécuter plusieurs formulaires**, en conservant toutes les données sensibles sur vos serveurs, ou d'avoir **votre propre form builder WYSIWYG hébergé** que les utilisateurs non techniques peuvent utiliser facilement.

Vous pouvez également **analyser les résultats des sondages** dans des tableaux de bord personnalisés, enregistrer vos formulaires au format PDF ou simplement **les convertir en fichiers PDF modifiables**. Il offre un support natif pour **React et Angular**, et la possibilité de choisir le backend de son choix: **toute combinaison serveur + base de données est possible**.

---

## 📱 React-Native {#react-native}

[Expo SDK 48 beta is now available](https://blog.expo.dev/expo-sdk-48-beta-is-now-available-7ee5713d0937)

https://twitter.com/Baconbrix/status/1623803911290118144

---

- 🐦 [Sebastian Markbåge about Server Components: "one of the reasons it was even funded by Meta initially was because of the promise it saw using server-driven technologies on native"](https://twitter.com/sebmarkbage/status/1623175042971344896): les React-Native Server Components ont bien un intérêt. Expo est sans doute le mieux placé pour les implémenter.
- 📊 [State of React-Native 2022](https://results.stateofreactnative.com/): les résultats du sondage sont disponibles.
- 📜 [Expo Changelog: January 2023](https://blog.expo.dev/expo-changelog-january-2023-c5eab76df08)
- 📜 [Expo - Sunsetting “expo publish” and Classic Updates](https://blog.expo.dev/sunsetting-expo-publish-and-classic-updates-6cb9cd295378)
- 👀 [React-Navigation 7 alpha](https://twitter.com/reactnavigation/status/1626648568189620238): static configuration API
- 💬 [Moving Fastlane to Mobile Native Foundation?](https://github.com/MobileNativeFoundation/discussions/discussions/194)
- 🐦 [SpaceX Starlink using Expo](https://twitter.com/sebastienlorber/status/1626150388704452611)
- 🎙️ [React-Native-Radio 259 - Tamagui with Nate Weinert](https://reactnativeradio.com/episodes/rnr-259-tamagui-with-nate-weinert)
- 📦 [@expo/html-elements@0.4.1](https://twitter.com/Baconbrix/status/1625984235713581057)
- 📦 [React-Native-MKKV 2.7.0 - fastWrites prop](https://twitter.com/mrousavy/status/1625059908927234048)
- 📦 React-Native-Skia - [Headless Support](https://twitter.com/wcandillon/status/1622558366940778496) + [Web-only support](https://github.com/Shopify/react-native-skia/pull/1336): la librairie est en train de devenir plus "universelle".
- 📦 [expo-github-action v8](https://twitter.com/cedricvanputten/status/1627732937008349186)
- 📜 [React Native at STRV in 2023](https://www.aronberezkin.com/posts/react-native-at-strv-in-2023)
- 📜 [How to use React Native Web with Storybook v7 beta](https://dev.to/dannyhw/how-to-use-react-native-web-with-storybook-v7-beta-1io7)

---

## 🧑‍💻 Jobs {#jobs}

🧑‍💼 [**Passionfroot - Senior Full-stack Engineer (Remix) - €160k+, Berlin/remote**](https://passionfroot.recruitee.com/o/senior-fullstack-engineer)

Passionfroot's mission is to empower the independent businesses of tomorrow via YouTube, Podcasts, Social Media, and Newsletters. Join us in building a tool that will empower creators globally to build scalable, sustainable businesses.

🧑‍💼 [**Shotgun - Senior Full Stack Engineer - 60-75k€ + BSPCE**](https://shotgun.teamtailor.com/jobs/1981578-senior-full-stack-engineer)

Shotgun est la billetterie des évènements electro. Nous recherchons des devs expérimentés pour travailler sur une stack moderne: React, React-Native, Typescript, GraphQL, NextJS, PgSQL, Vercel...

🧑‍💼 [**Premier Octet - Développeurs(euses) React - Paris**](https://www.premieroctet.com/jobs)

Nous cherchons des développeurs(euses) JavaScript passionnés. Vous interviendrez en développement sur des projets variés et motivants, à la fois serveur et front (React, Nest, Next), et éventuellement mobile (React Native).

🧑‍💼 [**Teale - Mobile Software Engineer - 50-60k - Paris/Remote**](https://sebastienlorber.mylittleteam.com/teale/multifunctional/mobile-software-engineer)

Teale recrute un dev mobile React-Native / TypeScript pour travailler sur une app à fort impact dédiée à la santé mentale.

💡 **[Comment publier une offre d'emploi ?](https://thisweekinreact.com/fr/sponsor)**

🏢 **[Vous recrutez en CDI ou freelance? Contactez-moi!](https://qq4ph6n1rk0.typeform.com/to/qjSZ38dQ#email={{subscriber.email_address}})**

---

## 🇫🇷 En français {#fr}

- 🎥 [React-Native-Paris - talks du 8 Février](https://www.youtube.com/playlist?list=PLZsRQnRG-mlJiCuv9q1nBkxaYP9AIl_Rn): 2 vidéos, sourcemaps et react-native-testing-library
- 🎥 [React Query rend ton Code 200% Plus Simple](https://www.youtube.com/watch?v=3jlBocCieVU)

💡Tu peux m'envoyer ton contenu: [sebastien@thisweekinreact.com](mailto:sebastien@thisweekinreact.com?subject=Contenu%20en%20fran%C3%A7ais) !

<Todo />

---

## 🔀 Autre {#other}

- [ts-reset](https://github.com/total-typescript/ts-reset): idée intéressante, sorte de CSS reset pour TypeScript qui patch certaines parties pénibles. Par exemple, `.filter(Boolean)` devient un type-guard.
- [rspack](https://www.npmjs.com/package/@rspack/core) + [oxc](https://github.com/Boshen/oxc): des devs ByteDance (TikTok) travaillent sur de nouveaux outils Rust, équivalent de Webpack/Babel
- [WebContainer API is here](https://blog.stackblitz.com/posts/webcontainer-api-is-here/) + https://twitter.com/stackblitz/status/1625552724547993600
- [Web Push for Web Apps on iOS and iPadOS](https://webkit.org/blog/13878/web-push-for-web-apps-on-ios-and-ipados/)
- [WebKit - Try out CSS Nesting today](https://webkit.org/blog/13813/try-css-nesting-today-in-safari-technology-preview/)
- [Container queries land in stable browsers](https://web.dev/cq-stable/)
- [TypeScript Course - Build a TypeScript Project From Scratch (free)](https://www.typescriptcourse.com/tutorials/build-a-typescript-project-from-scratch)
- [HOTScript - Higher-Order TypeScript](https://github.com/gvergnaud/hotscript) + https://twitter.com/GabrielVergnaud/status/1624838833941381122
- [Understanding Monomorphism to Improve Your JS Performance Up to 60x](https://www.builder.io/blog/monomorphic-javascript)
- [Vercel Cron Jobs](https://twitter.com/leeerob/status/1625249090932994051)
- [Declarative Shadow DOM](https://webkit.org/blog/13851/declarative-shadow-dom/)
- [Rust to WebAssembly the hard way](https://surma.dev/things/rust-to-webassembly/)
- [Lighthouse 10.0](https://developer.chrome.com/blog/lighthouse-10-0/)
- [Safari Technology Preview 163](https://webkit.org/blog/13839/release-notes-for-safari-technology-preview-163/)
- [Bringing Javascript to WebAssembly for Shopify Functions](https://shopify.engineering/javascript-in-webassembly-for-shopify-functions)
- [How Deno’s Founder Is Rebuilding the Runtime of the Web](https://www.sequoiacap.com/article/deno-spotlight/)

---

## 🤭 Fun {#fun}

[![alt](/emails/issues/135/meme.jpg)](https://twitter.com/t3dotgg/status/1626393874393595907)

A plus 👋
