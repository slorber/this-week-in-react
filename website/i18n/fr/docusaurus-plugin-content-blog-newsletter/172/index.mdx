...
- La collecte de données est une catégorie où les résultats sont conformes à l'attention qui peut être observée sur les médias sociaux, avec TanStack Query et tRPC qui occupent les premières places en termes d'intérêt et de rétention, et le propre Relay de Meta qui reste pratiquement sans intérêt en dehors de Meta avec seulement 5 % d'utilisation.
- L'avenir de la navigation est Expo Router, qui a explosé de 12 % d'utilisation l'année dernière à 40 % aujourd'hui.
- Si l'on en juge par l'activité des médias sociaux, le style semble être la catégorie la plus discutée, avec une abondance d'options. En termes d'utilisation, les classiques (StyleSheet API, inline styling, styled components) restent les options dominantes, avec NativeWind et Tamagui qui les rattrapent. Tamagui a réussi à susciter le plus d'intérêt au cours de l'année écoulée (et a également le plus de rétention et d'intérêt en tant que bibliothèque de composants d'interface utilisateur), suivi de près par un nouveau concurrent : react-native-unistyles.
- Un coup de chapeau spécial à Expo : Dans la section "Outils React Native", 6 des 10 meilleurs outils ainsi que les 2 meilleures options dans la catégorie déploiement sont fabriqués par Expo.
- Plus de 85 % des répondants pensent que React Native évolue dans la bonne direction et la part des développeurs qui trouvent que la création d'applications RN est trop complexe actuellement a diminué d'environ 30 % à 25 % - c'est encore trop, mais nous semblons évoluer dans la bonne direction !

Dans l'ensemble, des résultats vraiment intéressants qui méritent d'être [vérifiés] (https://results.stateofreactnative.com/) en détail ! Un grand merci à Bartłomiej, Kacper et Software Mansion pour avoir rendu cela possible !
