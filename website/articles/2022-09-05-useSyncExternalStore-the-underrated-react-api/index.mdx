...
You might have heard of [`useSyncExternalStore()`](https://reactjs.org/docs/hooks-reference.html#usesyncexternalstore), a new React 18 hook to **subscribe to external data sources**. This hook is often used internally by state management libraries - like [Redux](https://github.com/reduxjs/react-redux/pull/1808) - to implement a **selector system** that is [compatible with React 18 Concurrent features](https://github.com/reactwg/react-18/discussions/70).

But what about using `useSyncExternalStore()` in your own application code?

In this interactive article, I want to you a problem you might already have: **over-returning React hooks triggering useless React re-renders**. We will see how `useSyncExternalStore()` can be a good solution.
...
The goal is not to criticize React-Router, but rather to illustrate the problem. `useLocation()` is just a good pragmatic candidate to create this interactive blog post. Your own React hooks and other third-party libraries might also over-return.
