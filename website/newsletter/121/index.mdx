---
slug: /121
title: "This Week In React #121: Next.js, experimental_cache, Qwikify, Typesafety, 3D, Vitest Preview, Outstatic, Remix, React-Native, Web Components..."
authors: [slorber]
date: 10-25-2022
...
Last week I made a mistake, I sent out the 220 edition instead of 120 😅 No one noticed, can't be that big a deal.
...
Kent C. Dodds explains the interest of having a typed app from end to end, from the DB to the UI, through the different limits: network, localstorage, DB... He talks about validation (schema, type guards), type generation (Prisma) and conventions (Remix). He explains how this experience is possible with Remix, thanks to inference: it is better to use `typeof` than `as`.
...
- 📜 [Currying Layout Component Patterns in Next.js](https://creativedesignsguru.com/currying-nextjs-layout/): suggest to parametrize the `getLayout` de Next.js to create Layout variants without leading to unmount/remount. Nice trick today, might become less relevant with the upcoming nested layouts support.
