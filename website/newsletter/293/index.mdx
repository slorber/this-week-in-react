---
slug: /293
title: "This Week In React #293: Next.js 16.3, browser() API, useOptimistic, Generative UI, MobX, SWR, React Aria, TanStack Charts | PlainText, Vision Camera, Gesture Handler, Expo Simulators, Firebase | Stacked PRs, Node, Vite, pnpm, Hono"
...
[Next.js 16.3 Released: Instant Navigations & 90% RAM Reduction](https://nextjs.org/blog/next-16-3)

Next.js 16.3 officially shipped with major engine and runtime upgrades.
Long dev sessions now use up to 90% less RAM, repeat builds leverage
artifact caching, and SSR handles up to 22% more requests. TypeScript 7
is supported natively out of the box, alongside built-in glob imports
via Turbopack.

The core release focuses on Instant Navigations (built on top of Cache
Components) to give server-driven apps the snappy responsiveness of an
SPA. This signals a clear architectural shift for Next.js: returning to
a dynamic-by-default model with fully explicit, predictable caching.
...
- 📜 [React 19's useActionState: Preventing Sequenced Double Submits](https://shubhra.dev/tutorials/react-19-useactionstate) \- explains how React 19's `useActionState` queue behaves and how to correctly handle `isPending` states to prevent duplicate submits.
...
- 📜 [Architectural Patterns for Generative UI in React](https://boda.sh/blog/generative-ui/) \- break down of three approaches to letting LLMs build UI dynamically — schema-constrained output, a declarative JSON spec (A2UI/AG-UI), and open-ended sandboxed code generation.
- 📦 [TanStack Charts \- Minimal (\~8 KiB core), framework-agnostic charting primitives on D3, with first-class React bindings \- Pre-alpha, but shipping daily updates.](https://github.com/TanStack/charts)
- 📦 [use-webmcp-tool \- React abstraction for exposing client-side tools to browser AI agents via WebMCP](https://github.com/GoogleChromeLabs/use-webmcp-tool)
...
- 📦 [MobX 7.0 \- Drops legacy compatibility, cutting ESM bundle size to 13.96 KiB gzip (10.32 KiB tree-shaken) while maintaining the mobx-react-lite vs mobx-react architecture](https://github.com/mobxjs/mobx/releases/tag/mobx%407.0.0)
...
- 📦 [React Hook Form 7.84 \- Enhanced \<Form /\> submit behavior, better performance and smaller bundle size](https://github.com/react-hook-form/react-hook-form/releases/tag/v7.84.0)
...
- 👀 [Simulators for agents from Expo coming soon](https://expo.dev/services/simulators) \- Expo announced an early access waitlist for on-demand cloud simulators for AI agents to build, run, and visually verify app changes.
- 📖 [Gesture Handler \- New interactive docs on callbacks & events](https://docs.swmansion.com/react-native-gesture-handler/docs/fundamentals/callbacks-events/#gestureevent-callbacks) \- you can now see the callbacks chain and get better understanding on how they resolve.
- 🐦 [React Native Plain Text](https://x.com/mdj_dev/status/2082453726707478622) \- a faster, lower memory alternative to use for simple texts. See how it performs in comparison to RawText in [the author's benchmark](https://x.com/mdj_dev/status/2082461349196534119). It's limited to static, single-style text right now, but something to keep an eye on!
...
- 📦 [Enriched HTML 1.1 is out \- Stable web support with full feature parity](https://github.com/software-mansion/react-native-enriched-html/releases/tag/v1.1.0)
- 📦 [React Native Firebase App 26.1 \- Migrated to Turbo Modules, requires new architecture](https://github.com/invertase/react-native-firebase/blob/main/packages/app/CHANGELOG.md)
...
- 📦 [Sentry 8.21 \- Improved session replay details](https://github.com/getsentry/sentry-react-native/releases/tag/8.21.0)
...
- 🎥 [Build a React Native News Agent With Eve](https://www.youtube.com/watch?v=fEn-PQXBp28)
- 🎥 [Vision Camera 5 under the hood](https://www.youtube.com/watch?v=xI3egb3_ihI)

---

## 🔀 Other {#other}

- 📣 [GitHub stacked PRs are now in public preview! Adding native PR stacking with automatic base retargeting, partial merges, and official CLI (gh-stack) integration.](https://github.blog/changelog/2026-07-30-stacked-pull-requests-are-now-in-public-preview/)
- 📜 [Styling the Navigation: Declarative Route and Navigation Matching in CSS](https://www.bram.us/2026/07/30/styling-the-navigation-declarative-route-and-navigation-matching-in-css/) \- Bramus walks through a proposed CSS spec that moves route-based view-transition logic out of JS.
...
- 📦 [Node 24.19 \- Stabilized `stream.compose`, new TLS/TCP configuration options and `--experimental-import-text` flag, custom buffers in `fs.readFile()`](https://nodejs.org/en/blog/release/v24.19.0)
...
- 📦 [Vite 8.2 \- Adds a top-level input option (bypassing `build.rolldownOptions`), warnings for unsupported native loader features, PostCSS config types, and dev URLs labeled by interface](https://github.com/vitejs/vite/blob/v8.2.0/packages/vite/CHANGELOG.md#820-2026-07-30)
- 📦 [Hono 4.13 \- Delivers up to 1.25x performance gains, adds HTTP QUERY method support, a Method Not Allowed middleware, and various core improvements](https://github.com/honojs/hono/releases/tag/v4.13.0)
