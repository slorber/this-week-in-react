...
If a title is too long, click-bait or weirdly capitalized, refactor it so that all titles are consistent.

For releases, we drop the “v” and the patch number (3.1, not v3.1.9). Include the major release highlight keywords directly in the link, separated with a dash `-`. Don’t include a release if there’s nothing interesting to highlight, which is often the case for patch releases. What’s “inside” the link is the “objective” part (the highlights or formal project description), what’s “outside” is your subjective comment (if any). Examples:
...
If it’s a new project or something we feature for the first time, and replace keywords by an objective project description (inspired by GitHub repo description, landing page, SEO tags). Even if there's an introduction blog post, we format it as a release. Examples:
...
- 🔗: resources
- 🤖: tools for AI / agents (skills, CLIs, MCPs ...)
- 🎥: videos
- 🎙️: podcasts
...
- For a given "emoji group", put the most interesting links first, usually with a comment
- It's better not to comment on something than to make the wrong comment, or a useless comment.
